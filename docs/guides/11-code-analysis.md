...
`src/lang/analyze.py`, `src/lang/analysis_model.py`, and `src/lang/analysis_cfg.py` separate project loading/CLI, AST analysis/modeling, CFG construction, call resolution, and graph rendering. Additional analyses should consume the shared model or add fields to it rather than reparsing source independently. Natural extensions include data-flow/liveness, def-use chains, constant propagation views, dominators, loop nesting/irreducibility checks, typed call resolution, and editor/LSP visualization hooks.
