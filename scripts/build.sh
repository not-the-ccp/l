...
build_one() {
    kind=$1 out=$2 dest=$3
    "$PYTHON" "$HERE/src/lang/native_compile.py" --tool "$kind" --cc "$CC" -o "$dest/$out"
}
build_lace() {
    dest=$1
    CC="$CC" "$HERE/scripts/lc" --root "$HERE" "$HERE/tools/lace2/main.l" -o "$dest/lace" >/dev/null
}
build_shell() {
    dest=$1
...
build_syntax() {
    dest=$1
    CC="$CC" "$HERE/scripts/lc" "$HERE/tools/syntax/lsyntax.l" -o "$dest/lsyntax" >/dev/null
}
build_check() {
    dest=$1
