...
from lang import HostModule, OpaqueVal, SomeVal, TrapSig, const_arr, name_ty, opt
from hosts.linux_host import (
    CHILD_TYPE,
    GROUP_TYPE,
    LinuxChild,
    LinuxGroup,
...
class LinuxJobHost:
    ...
    def _wait_child(self, value):
        ...
        if child.pidfd >= 0 and hasattr(os, "P_PIDFD"):
            result = self._waitid(os.P_PIDFD, child.pidfd, False)
        else:
            result = self._waitid(os.P_PID, child.pid, False)
        if result is None:
            raise TrapSig("blocking child wait returned no event")
        return result
