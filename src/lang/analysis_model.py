...
class FunctionAnalysis:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "cfg": {
                "nodes": [asdict(x) for x in self.cfg_nodes],
                "edges": [asdict(x) for x in self.cfg_edges],
            },
        }
    ...
    def to_dict(self):
        ...
        return {
            ...
            "summary": {
                ...
                "functions": len(fs),
                "statements": sum(f.metrics["statements"] for f in fs),
                "decisions": sum(f.metrics["decisions"] for f in fs),
                "loops": sum(f.metrics["loops"] for f in fs),
                "calls": sum(len(f.calls) for f in fs),
                "local_bindings": sum(f.metrics["local_bindings"] for f in fs),
                "rebound_local_bindings": sum(
                    f.metrics["rebound_local_bindings"] for f in fs
                ),
                "unreachable_statements": sum(len(f.unreachable_lines) for f in fs),
            },
...
def walk(node: N, descend_anon=False):
    ...
    if node.kind == "anonfn" and not descend_anon:
        return
    for child in children(node.a):
        yield from walk(child, descend_anon)
        ...
        for n in walk(stmt):
            if n.kind == "var":
                ...
                binding = LocalBinding(
                    name, getattr(sp, "line", None), None if ty is None else str(ty)
                )
...
def metrics(body, max_nesting, bindings):
    ...
    return {
        ...
        "decisions": decisions,
        "loops": c["while"] + c["for"] + c["forin"],
        "matches": c["match"],
        "returns": c["return"],
        "traps": c["trap"],
        "breaks": c["break"],
        "continues": c["continue"],
        "short_circuit_ops": short,
        "cyclomatic_complexity": 1 + decisions,
        "max_nesting": max_nesting,
        "local_bindings": len(bindings),
        "rebound_local_bindings": rebound,
...
def analyze_fn(module, name, params, ret, body, node, source, anonymous=False):
    ...
    fn = FunctionAnalysis(
        module,
        name,
        [{"name": n, "type": str(t)} for n, t in params],
        str(ret),
        getattr(sp, "line", None),
        getattr(sp, "end_line", None),
        metrics(body, nesting, bindings),
        collect_calls(body),
        bindings,
        nodes,
        edges,
    ...
    for anon in find_anons(body):
        a = getattr(anon, "span", None)
        nested.append(
            (f"{name}::<anon@{getattr(a,'line','?')}:{getattr(a,'col','?')}>", anon)
        )
    return fn, nested
...
def analyze_project(sources, entry):
    ...
    for modname in sorted(sources):
        ...
        for d in ast.a[0]:
            ...
            if d.kind == "import":
                q, alias = d.a
                m.imports[alias or q[-1]] = tuple(q)
            elif d.kind == "fn":
                _, name, _, params, ret, body = d.a
                fn, nested = analyze_fn(modname, name, params, ret, body, d, source)
                m.functions.append(fn)
                pending += nested
        while pending:
            name, anon = pending.pop(0)
            params, ret, body = anon.a
            fn, nested = analyze_fn(
                modname, name, params, ret, body, anon, source, True
            )
            m.functions.append(fn)
            pending += nested
    p = ProjectAnalysis(entry, modules, asts)
    resolve_calls(p)
    return p
...
def resolve_calls(project):
    ...
    for m in project.modules:
        ...
        for fn in m.functions:
            for c in fn.calls:
                if c.callee == "<indirect>":
                    continue
                parts = c.callee.split(".")
                target = None
                if len(parts) == 1 and parts[0] in local:
                    target = f"{'.'.join(m.name)}::{parts[0]}"
                elif len(parts) >= 2 and parts[0] in m.imports:
                    target = f"{'.'.join(m.imports[parts[0]])}::{'.'.join(parts[1:])}"
                elif len(parts) >= 2:
                    target = f"{'.'.join(parts[:-1])}::{parts[-1]}"
                else:
                    target = c.callee
                c.resolved = target
                c.internal = target in index
...
def ast_value(value):
    ...
    if isinstance(value, N):
        ...
        return {
            "kind": value.kind,
            "span": span,
            "type": None if getattr(value, "ty", None) is None else str(value.ty),
            "args": [ast_value(x) for x in value.a],
        }
    if isinstance(value, (tuple, list)):
        return [ast_value(x) for x in value]
    if isinstance(value, bytes):
        return {"bytes_hex": value.hex()}
    if hasattr(value, "kind") and value.__class__.__name__ == "Ty":
        return str(value)
...
def select_functions(project, module=None, function=None):
    ...
    if module:
        ...
        if not fs:
            raise LangError(f"analysis module not found: {module}")
    if function:
