        ...
        while i < n:
            ...
            if s.startswith("//", i):
                ...
                if self.keep_comments:
                    out.append(
                        Tok("COMMENT", txt, span(st, sl, sc, j, line, col + len(txt)))
                    )
                advance(txt)
                i = j
                continue
            ...
            if c.isascii() and (c.isalpha() or c == "_"):
                i += 1
                while i < n and s[i].isascii() and (s[i].isalnum() or s[i] == "_"):
                    i += 1
                txt = s[st:i]
                advance(txt)
                out.append(
                    Tok(
                        txt if txt in KEYWORDS else "NAME",
                        txt,
                        span(st, sl, sc, i, line, col),
                    )
                )
                continue
            ...
            if c.isdigit():
                ...
                if c == "0" and i < n and s[i] in "xXbB":
                    base = s[i]
                    i += 1
                    valid = "01" if base in "bB" else "0123456789abcdefABCDEF"
                    if i >= n or s[i] not in valid:
                        raise LangError(
                            "expected digit after numeric base prefix",
                            span(st, sl, sc, i, line, col + (i - st)),
                        )
                    prev_digit = False
                    while i < n:
                        ch = s[i]
                        if ch in valid:
                            ...
                            ch == "_" and prev_digit and i + 1 < n and s[i + 1] in valid
                        ):
                            prev_digit = False
                            i += 1
                        else:
                    ...
                    txt = s[st:i]
                    advance(txt)
                    out.append(Tok("INT", txt, span(st, sl, sc, i, line, col)))
                    continue
                ...
                def digits(pos):
                    ...
                    while p < n:
                        if s[p].isdigit():
                            prev = True
                            p += 1
                        elif s[p] == "_" and prev and p + 1 < n and s[p + 1].isdigit():
                            prev = False
                            p += 1
                        else:
                            break
            ...
            if c in "'\"":
                ...
                while i < n and s[i] != quote:
                    ...
                    if s[i] == "\\":
                        ...
                        if e == "x":
                            ...
                            raise LangError(f"unknown escape \\{e}")
                    else:
                ...
                if quote == "'":
                    if len(raw) != 1:
                        raise LangError(
                            "byte literal must encode exactly one byte",
                            span(st, sl, sc, i, line, col),
                        )
                    out.append(
                        Tok("BYTE", str(raw[0]), span(st, sl, sc, i, line, col), txt)
                    )
                else:
                    out.append(
                        Tok(
            ...
            if matched:
                i += len(matched)
                advance(matched)
                out.append(Tok(matched, matched, span(st, sl, sc, i, line, col)))
                continue
            if c in SINGLE:
                i += 1
                advance(c)
                out.append(Tok(c, c, span(st, sl, sc, i, line, col)))
                continue
            raise LangError(
                f"unexpected character {c!r}", span(st, sl, sc, i + 1, line, col + 1)
            )
        eof = Span(n, n, line, col, line, col)
...
def same_type(a, b):
    ...
    if a.kind == "name":
        aq, aa = a.a
        bq, ba = b.a
        return aq == bq and len(aa) == len(ba) and all(same_type(x, y) for x, y in zip(aa, ba))
    return a == b
...
class Parser:
    ...
    def node(self, k, *a, st=None, en=None):
        ...
        if en is None:
            en = self.ts[max(0, self.i - 1)].span
        sp = (
            Span(st.start, en.end, st.line, st.col, en.end_line, en.end_col)
            if st and en
            else None
        )
    ...
    def program(self):
        ...
        return N(
            "module",
            (tuple(ds),),
            Span(0, len(self.src), 1, 1, self.t().span.line, self.t().span.col),
        )
    ...
    def decl(self):
        ...
        if self.maybe("import"):
            if public:
                raise LangError("imports cannot be pub", st)
            q = self.qname()
            alias = None
            if self.maybe("as"):
                alias = self.take("NAME").text
            en = self.take(";").span
            return self.node("import", q, alias, st=st, en=en)
        if self.maybe("const"):
        ...
        if self.maybe("struct"):
            ...
            fs = self.comma_list("}", fld)
            en = self.take("}").span
            return self.node("struct", public, n, gps, tuple(fs), st=st, en=en)
        if self.maybe("enum"):
            n = self.take("NAME").text
            gps = self.type_params()
            self.take("{")
            ...
            def var():
                ...
                payload = ()
                if self.maybe("("):
                    payload = tuple(self.comma_list(")", self.type))
                    self.take(")")
                return (v, payload)

            vs = self.comma_list("}", var)
            en = self.take("}").span
            return self.node("enum", public, n, gps, tuple(vs), st=st, en=en)
        if self.maybe("fn"):
            n = self.take("NAME").text
            gps = self.type_params()
            ps = self.params()
            ...
            body, en = self.block()
            return self.node(
                "fn", public, n, gps, tuple(ps), rt, tuple(body), st=st, en=en
            )
    ...
    def type(self):
        ...
        if self.maybe("const"):
            ...
            if not self.maybe("["):
                raise LangError(
                    "const in a type must qualify an array: expected []", start
                )
            self.take("]")
        ...
        args = ()
        if self.maybe("["):
            args = tuple(self.comma_list("]", self.type))
            self.take("]")
    ...
    def stmt(self):
        ...
        if self.maybe("var"):
            ...
            e = self.expr()
            en = self.take(";").span
            return self.node("var", n, ty, e, st=st, en=en)
        if self.maybe("if"):
            self.take("(")
            c = self.condition()
            self.take(")")
        ...
        if self.maybe("while"):
            ...
            self.take(")")
            b, en = self.block()
            return self.node("while", c, tuple(b), st=st, en=en)
        if self.maybe("for"):
            self.take("(")
            ...
            if self.t("NAME") and self.ts[self.i + 1].kind == "in":
                ...
                self.take(")")
                b, en = self.block()
                return self.node("forin", n, e, tuple(b), st=st, en=en)
            init = None
            cond = None
            step = None
            if not self.t(";"):
            ...
            self.take(";")
            if not self.t(")"):
                step = self.assign_no_semi()
            self.take(")")
            b, en = self.block()
            return self.node("for", init, cond, step, tuple(b), st=st, en=en)
        if self.maybe("match"):
            self.take("(")
            e = self.expr()
            self.take(")")
        ...
        if self.maybe("return"):
            ...
            en = self.take(";").span
            return self.node("return", e, st=st, en=en)
        if self.maybe("break"):
            en = self.take(";").span
            return self.node("break", st=st, en=en)
        if self.maybe("continue"):
            en = self.take(";").span
            return self.node("continue", st=st, en=en)
        if self.maybe("trap"):
            en = self.take(";").span
            return self.node("trap", st=st, en=en)
        lhs = self.expr()
        if self.t().kind in ASSIGN:
            op = self.take().kind
            rhs = self.expr()
            en = self.take(";").span
            return self.node("assign", lhs, op, rhs, st=st, en=en)
        en = self.take(";").span
    ...
    def assign_no_semi(self):
        ...
        if op not in ASSIGN:
            raise LangError(
                "expected assignment in for clause", self.ts[self.i - 1].span
            )
        rhs = self.expr()
        return self.node("assign", lhs, op, rhs, st=st, en=rhs.span)
    ...
    def payload_pattern(self):
        ...
        if self.t("NAME") and self.t().text == "_":
            self.take()
            return self.node("p_wild", st=st)
        if self.t("NAME"):
            x = self.take()
            return self.node("p_bind", x.text, st=st, en=x.span)
    ...
    def pattern(self):
        ...
        if self.t("NAME") and self.t().text == "_":
            ...
            return self.node("p_wild", st=st)
        if self.maybe("none"):
            return self.node("p_none", st=st)
        if self.maybe("some"):
            if self.maybe("("):
                p = self.payload_pattern()
                en = self.take(")").span
            else:
                p = self.node("p_wild", st=st)
                en = self.ts[self.i - 1].span
            return self.node("p_some", p, st=st, en=en)
        if self.maybe("("):
            en = self.take(")").span
            return self.node("p_unit", st=st, en=en)
        if self.maybe("true"):
            return self.node("p_bool", True, st=st)
        if self.maybe("false"):
            return self.node("p_bool", False, st=st)
        if self.t("INT"):
            x = self.take()
            return self.node("p_int", x.text, st=st, en=x.span)
        if self.t("BYTE"):
            x = self.take()
            return self.node("p_byte", int(x.text), st=st, en=x.span)
        if self.t("NAME"):
            q = self.qname()
            ps = ()
            if self.maybe("("):
                ps = tuple(self.comma_list(")", self.payload_pattern))
                en = self.take(")").span
            else:
                en = self.ts[self.i - 1].span
    ...
    def expr(self, minp=0):
        ...
        if self.maybe("("):
            if self.maybe(")"):
                left = self.node("unit", st=st, en=self.ts[self.i - 1].span)
            else:
                left = self.expr()
                self.take(")")
        elif self.t("INT"):
            x = self.take()
            left = self.node("int", x.text, st=st, en=x.span)
        elif self.t("FLOAT"):
            x = self.take()
            left = self.node("float", x.text, st=st, en=x.span)
        elif self.t("BYTE"):
            x = self.take()
            left = self.node("byte", int(x.text), st=st, en=x.span)
        elif self.t("STRING"):
            x = self.take()
            left = self.node("string", bytes.fromhex(x.text), st=st, en=x.span)
        elif self.maybe("true"):
            left = self.node("bool", True, st=st)
        elif self.maybe("false"):
            left = self.node("bool", False, st=st)
        elif self.maybe("none"):
            left = self.node("none", st=st)
        elif self.maybe("some"):
            self.take("(")
            x = self.expr()
            en = self.take(")").span
            left = self.node("some", x, st=st, en=en)
        elif self.maybe("new"):
            x = self.expr(10)
            left = self.node("new", x, st=st, en=x.span)
        elif self.maybe("fn"):
            ps = self.params()
            rt = UNIT
            if self.maybe("->"):
                rt = self.type()
            b, en = self.block()
            left = self.node("anonfn", tuple(ps), rt, tuple(b), st=st, en=en)
        elif self.maybe("["):
            if self.maybe("]"):
                left = self.node("array", tuple(), st=st, en=self.ts[self.i - 1].span)
            else:
                first = self.expr()
                if self.maybe(";"):
                    n = self.expr()
                    en = self.take("]").span
                    left = self.node("repeat", first, n, st=st, en=en)
                else:
                    xs = [first]
                    while self.maybe(","):
                        if self.t("]"):
                        ...
                        xs.append(self.expr())
                    en = self.take("]").span
                    left = self.node("array", tuple(xs), st=st, en=en)
        elif self.t("NAME"):
            q = self.qname()
            left = self.node("qname", q, st=st, en=self.ts[self.i - 1].span)
        elif self.t().kind in ("!", "~", "-", "*"):
            opx = self.take().kind
            x = self.expr(10)
            left = self.node("unary", opx, x, st=st, en=x.span)
        else:
            raise LangError("expected expression", self.t().span)
        # postfix: call, index, field, struct literal
        while True:
            if self.maybe("("):
                args = tuple(self.comma_list(")", self.expr))
                en = self.take(")").span
                left = self.node("call", left, args, st=st, en=en)
                continue
            if self.maybe("["):
                save = self.i
                try:
                    type_args = tuple(self.comma_list("]", self.type))
                    # comma_list does not consume the close delimiter;
                    # verify that the next token is ] after parsing type args.
                    if not self.t("]"):
                        self.i = save
                    else:
                        self.take("]")
                        if self.maybe("("):
                            call_args = tuple(self.comma_list(")", self.expr))
                            en = self.take(")").span
                            left = self.node(
                                "generic_call", left, type_args, call_args,
                                st=st, en=en,
                            )
                            continue
                        else:
                    ...
                    self.i = save
                # Fall through: parse as index expression
                ix = self.expr()
                en = self.take("]").span
                left = self.node("index", left, ix, st=st, en=en)
                continue
            if self.maybe("."):
                n = self.take("NAME")
                left = self.node("field", left, n.text, st=st, en=n.span)
                continue
            if self.maybe("{"):
                fs = []
                if not self.t("}"):
                    while True:
                        ...
                        if self.t("}"):
                            break
                en = self.take("}").span
                left = self.node("structlit", left, tuple(fs), st=st, en=en)
                continue
            break
        if self.maybe("as"):
            t = self.type()
            left = self.node("cast", left, t, st=st, en=self.ts[self.i - 1].span)
        used_cmp = False
        while self.t().kind in PREC or self.t("is"):
            opx = self.t().kind
            p = 3 if opx == "is" else PREC[opx]
            if p < minp:
                break
            if opx in CMP or opx == "is":
                if used_cmp:
                    raise LangError(
                        "comparison / is operators do not chain", self.t().span
                    )
                used_cmp = True
            self.take()
            if opx == "is":
                pat = self.pattern()
                left = self.node("is", left, pat, st=st, en=pat.span)
            else:
                right = self.expr(p + 1)
                left = self.node("binary", opx, left, right, st=st, en=right.span)
        return left
...
def substitute(t: Ty | ConstArrayTy, m: dict[str, Ty]) -> Ty | ConstArrayTy:
    ...
    if t.kind == "param":
        return m.get(t.a[0], t)
    if t.kind == "name":
        q, args = t.a
        return Ty("name", (q, tuple(substitute(x, m) for x in args)))
    if t.kind in ("opt", "ref", "array"):
        return Ty(t.kind, (substitute(t.a[0], m),))
    if t.kind == "fn":
        return fnty([substitute(x, m) for x in t.a[0]], substitute(t.a[1], m))
    return t
...
def scalar_value(opx, a, b, t, fault=LangError):
    ...
    if opx == "!=":
        return (a is not b) if t.kind == "ref" else a != b
    if opx == "<":
        return a < b
    if opx == "<=":
        return a <= b
    ...
    if opx in ("<<", ">>"):
        w, _ = int_info(t)
        if b < 0 or b >= w:
            raise fault("invalid shift count")
        return wrap_int((a << b) if opx == "<<" else (a >> b), t)
    if opx in ("&", "|", "^"):
        v = {"&": lambda: a & b, "|": lambda: a | b, "^": lambda: a ^ b}[opx]()
        return wrap_int(v, t)
    if opx in ("+", "-", "*"):
        v = {"+": lambda: a + b, "-": lambda: a - b, "*": lambda: a * b}[opx]()
        return wrap_int(v, t) if int_info(t) else fround(v, t)
    if opx == "/":
        if int_info(t):
            if b == 0:
                raise fault("division by zero")
            q = abs(a) // abs(b)
    ...
    if opx == "%":
        if b == 0:
            raise fault("remainder by zero")
        q = abs(a) // abs(b)
        q = -q if (a < 0) ^ (b < 0) else q
        return wrap_int(a - q * b, t)
...
class StructInfo:
    ...
    name: str
    gps: tuple[str, ...]
    fields: tuple[tuple[bool, str, Ty, Span], ...]
    owner: tuple[str, ...]
...
class EnumInfo:
    ...
    name: str
    gps: tuple[str, ...]
    variants: tuple[tuple[str, tuple[Ty, ...]], ...]
    owner: tuple[str, ...]
...
class Checker:
    ...
    def __init__(
        ...
        self.place_types = {}
        self.anon_types = {}
        self.current_origin = self.module_name
    ...
    def collect(self):
        ...
        for d in self.mod.a[0]:
            ...
            if k == "import":
                ...
                public, n, gps, fs = d.a
                self.claim(n, d)
                self._unique(gps, "generic parameter", d)
                self._unique([x[1] for x in fs], "field", d)
                self.structs[n] = StructInfo(
                    public, n, gps, fs, tuple(getattr(d, "origin", self.module_name))
                )
            elif k == "enum":
                public, n, gps, vs = d.a
                self.claim(n, d)
                self._unique(gps, "generic parameter", d)
                self._unique([x[0] for x in vs], "variant", d)
                self.enums[n] = EnumInfo(
                    public, n, gps, vs, tuple(getattr(d, "origin", self.module_name))
                )
            elif k == "fn":
                public, n, gps, ps, rt, b = d.a
                self.claim(n, d)
                self._unique(gps, "generic parameter", d)
                self._unique([x[0] for x in ps], "parameter", d)
    ...
    def checked(self):
        ...
        for c in self.consts.values():
            c.ty = self.resolve_ty(c.ty)
        for f in self.funcs.values():
            self.gparams = set(f.gps)
            f.params = tuple((n, self.resolve_ty(t)) for n, t in f.params)
            f.ret = self.resolve_ty(f.ret)
        self.gparams = set()
    ...
    def resolve_ty(self, t: Ty | ConstArrayTy) -> Ty | ConstArrayTy:
        ...
        if t.kind == "unit":
            return t
        if t.kind in ("opt", "ref", "array"):
            return Ty(t.kind, (self.resolve_ty(t.a[0]),))
        if t.kind == "fn":
            return fnty([self.resolve_ty(x) for x in t.a[0]], self.resolve_ty(t.a[1]))
        if t.kind != "name":
            return t
        q, args = t.a
        if len(q) == 1 and q[0] in self.gparams:
            if args:
                self.err(f"type parameter {q[0]} cannot have type arguments")
            return tparam(q[0])
        args = tuple(self.resolve_ty(x) for x in args)
        if len(q) == 1 and q[0] in PRIMS:
            if args:
                self.err(f"primitive {q[0]} takes no type arguments")
            return Ty("name", (q, ()))
        if len(q) == 1 and q[0] in self.structs:
            need = len(self.structs[q[0]].gps)
            if len(args) != need:
                self.err(f"{q[0]} expects {need} type arguments, got {len(args)}")
        ...
        if len(q) >= 2 and q[0] in self.imports:
            target = self.imports[q[0]]
            hm = self.host_modules.get(target)
            if hm is not None:
                return hm.resolve_type(q[1:], args)
        if len(q) >= 2:
            return Ty("name", (q, args))
    ...
    def validate_layouts(self):
        ...
        def walk(t, env, path):
            ...
            if name in path:
                self.err(
                    "infinitely-sized recursive value type: "
                    + " -> ".join(path + [name])
                )
            args = t.a[1]
    ...
    def check_generic_recursion(self):
        ...
        def visit(n, owner):
            ...
            if n.kind == "call":
                ...
                if (
                    cal.kind == "qname"
                    and len(cal.a[0]) == 1
                    and cal.a[0][0] in generic
                ):
                    edges[owner].add(cal.a[0][0])
                    calls[owner].append((cal.a[0][0], n))
            for x in n.a:
                if isinstance(x, N):
                    visit(x, owner)
        ...
        def strong(v):
            ...
            if low[v] == ind[v]:
                ...
                if len(comp) > 1:
                    self.err(
                        "mutually recursive generic functions are not allowed: "
                        + ", ".join(sorted(comp))
                    )
    ...
    def bind(self, n, t, node=None):
        ...
        if (
            n in self.top
            or n in self.builtins
            or n in self.source_reserved
            or n in self.gparams
            or any(n in s for s in self.scopes)
        ):
            self.err(f"name {n} would shadow an existing name", node)
        self.scopes[-1][n] = t
    ...
    def check_fn(self, f: FnInfo):
        ...
        self.ret = f.ret
        self.current_fn = f
        self.current_origin = getattr(f.node, "origin", self.module_name)
        self.loop = 0
        self.scopes = []
        self.push()
        for n, t in f.params:
            self.bind(n, t, f.node)
        self.block(f.body, new=False)
        if f.ret != UNIT and not self.block_returns(f.body):
            self.err(f"function {f.name} may fall off the end", f.node)
        self.pop()
        self.gparams = set()
        self.current_fn = None
        self.source_reserved = set()
    ...
    def contains_break(self, ss):
        ...
        for s in ss:
            if s.kind == "break":
                return True
            if s.kind == "if":
                if self.contains_break(s.a[1]) or self.contains_break(s.a[2]):
                    return True
            if s.kind == "match":
    ...
    def stmt_returns(self, s):
        ...
        if s.kind == "if":
            return (
                bool(s.a[2])
                and self.block_returns(s.a[1])
                and self.block_returns(s.a[2])
            )
        if s.kind == "match":
            return bool(s.a[1]) and all(self.block_returns(b) for _, b in s.a[1])
        if (
            s.kind == "while"
            and s.a[0].kind == "bool"
            and s.a[0].a[0] is True
            and not self.contains_break(s.a[1])
        ):
            return True
        return False
    ...
    def stmt(self, s):
        ...
        if k == "var":
            ...
            if t is not None:
                ...
                et = self.expr(e)
                t = et
                if self.is_ambiguous_value(e):
                    self.err("initializer is ambiguous; add a type annotation", s)
            self.bind(n, t, s)
            return
        if k == "assign":
            l, opx, r = s.a
            lt = self.place(l)
            l.ty = lt
            rhs_expected = name_ty("u64") if opx in ("<<=", ">>=") else lt
            rt = self.expr(r, rhs_expected)
            if opx == "=":
                self.req(lt, rt, s)
            else:
                self.binop(opx[:-1], lt, rt, s)
            return
        if k == "exprstmt":
            if s.a[0].kind != "call":
                self.err("only calls may be expression statements", s)
            self.expr(s.a[0])
            return
        if k == "if":
            binds = self.condition(s.a[0])
            self.push()
            self.bindings(binds, s)
        ...
        if k == "for":
            ...
            if c:
                ...
                if binds:
                    self.err("for condition may not bind a pattern", c)
            if step:
                self.stmt(step)
            self.block(b)
            self.loop -= 1
        ...
        if k == "forin":
            ...
            at = self.expr(e)
            if at.kind != "array":
                self.err("for-in requires []T", e)
            self.push()
            self.loop += 1
            self.bind(n, at.a[0], s)
            self.block(b, False)
            self.loop -= 1
            self.pop()
            return
        if k == "match":
            ...
            for p, b in s.a[1]:
                ...
                if key == "_":
                    ...
                    self.err(f"duplicate match arm {key}", p)
                seen.append(key)
                self.push()
        ...
        if k == "return":
            e = s.a[0]
            if e is None:
                self.req(UNIT, self.ret, s)
            else:
                self.req(self.ret, self.expr(e, self.ret), s)
            return
        if k in ("break", "continue"):
            if not self.loop:
                self.err(f"{k} outside loop", s)
            return
        if k == "trap":
            return
    ...
    def condition(self, c):
        ...
        if c.kind == "is":
            ...
            return b
        self.req(name_ty("bool"), self.expr(c, name_ty("bool")), c)
        return {}
    ...
    def pattern(self, p, t: Ty):
        ...
        if k == "p_none":
            if t.kind != "opt":
                self.err("none pattern requires ?T", p)
            return {}, "none"
        if k == "p_some":
            if t.kind != "opt":
                self.err("some pattern requires ?T", p)
            b = self.payload_binding(p.a[0], t.a[0])
            return b, "some"
        if k == "p_bool":
            self.req(name_ty("bool"), t, p)
            return {}, str(p.a[0]).lower()
        if k == "p_int":
            if not int_info(t):
                self.err("integer pattern requires integer subject", p)
            return {}, ("int", parse_int_text(p.a[0]))
        if k == "p_byte":
            self.req(name_ty("u8"), t, p)
            return {}, ("byte", p.a[0])
        if k == "p_name":
            q, subs = p.a
            ...
            variant = self._variant_for_pattern(q, t)
            if variant is None:
                self.err("pattern name is neither binding nor matching enum variant", p)
            en, v, ts = variant
            # If no payloads were written but the variant has payloads,
            # fill in wildcards (tag-only check).
            if not subs:
                subs = tuple(N("p_wild") for _ in ts)
            if len(ts) != len(subs):
                self.err(f"{en}.{v} pattern expects {len(ts)} payloads", p)
            binds = {}
            for sp, pt in zip(subs, ts):
                b = self.payload_binding(sp, pt)
                if set(b) & set(binds):
                    self.err("duplicate binding in pattern", p)
                binds.update(b)
            return binds, (en, v)
    ...
    def _variant_for_pattern(self, q, t):
        ...
        if t.kind != "name" or len(t.a[0]) != 1:
            return None
        en = t.a[0][0]
        if en not in self.enums:
            return None
        info = self.enums[en]
        m = dict(zip(info.gps, t.a[1]))
        variants = dict(info.variants)
        if len(q) == 1:
            v = q[0]
        elif len(q) == 2 and q[0] == en:
            v = q[1]
        else:
            return None
        ...
        return (
            ...
            v,
            tuple(
                substitute(self._resolved_decl_ty(x, info.gps), m) for x in variants[v]
            ),
        )
    ...
    def check_exhaustive(self, t, seen, node):
        ...
        if t == UNIT:
            if "()" not in keys:
                self.err("non-exhaustive match: missing ()", node)
            return
        if is_prim(t, "bool"):
            miss = {"true", "false"} - keys
            if miss:
                self.err(
                    "non-exhaustive match: missing " + ", ".join(sorted(miss)), node
                )
            return
        if t.kind == "opt":
            miss = {"none", "some"} - keys
        ...
        if t.kind == "name" and len(t.a[0]) == 1 and t.a[0][0] in self.enums:
            en = t.a[0][0]
            needed = {(en, v) for v, _ in self.enums[en].variants}
            miss = needed - keys
            if miss:
                self.err(
                    "non-exhaustive match: missing "
                    + ", ".join(v for _, v in sorted(miss)),
                    node,
                )
            return
        self.err(
            "non-exhaustive match over an open value domain requires _ or a binding arm",
            node,
        )
    ...
    def expr(self, e: N, expected: Ty | None = None) -> Ty:
        ...
        if k == "unit":
            ...
            if expected and int_info(expected):
                w, s = int_info(expected)
                lo = -(1 << (w - 1)) if s else 0
                hi = (1 << (w - 1)) - 1 if s else (1 << w) - 1
                if not (lo <= v <= hi):
                    self.err(f"integer literal {v} does not fit {expected}", e)
                t = expected
            else:
                t = name_ty("i64")
            ...
            t = (
                expected
                if expected and expected.kind == "name" and expected.a[0][0] in FLOATS
                else name_ty("f64")
            )
            v = float(e.a[0].replace("_", ""))
            if not math.isfinite(v) or not math.isfinite(fround(v, t)):
                self.err(f"float literal out of finite range for {t}", e)
        elif k == "none":
            if not expected or expected.kind != "opt":
                self.err("none requires expected ?T", e)
            t = expected
        elif k == "some":
            ie = expected.a[0] if expected and expected.kind == "opt" else None
            it = self.expr(e.a[0], ie)
            t = opt(it)
        elif k == "qname":
            t = self.qname_expr(e.a[0], e, expected)
        elif k == "array":
            xs = e.a[0]
            elem = (
                expected.a[0]
                if expected and expected.kind == "array"
                else (self.expr(xs[0]) if xs else None)
            )
            if elem is None:
                self.err("empty array requires expected []T", e)
            for x in xs:
                self.req(elem, self.expr(x, elem), x)
            t = arr(elem)
        elif k == "repeat":
            x, n = e.a
            elem = (
                expected.a[0] if expected and expected.kind == "array" else self.expr(x)
            )
            self.req(elem, self.expr(x, elem), x)
            self.req(name_ty("u64"), self.expr(n, name_ty("u64")), n)
            t = arr(elem)
        elif k == "unary":
            opx, x = e.a
            if opx == "*":
                ...
                if (
                    ...
                    and x.kind == "int"
                    and expected
                    and int_info(expected)
                    and int_info(expected)[1]
                ):
                    w, _ = int_info(expected)
                    mag = parse_int_text(x.a[0])
                    if mag < 0 or mag > (1 << (w - 1)):
                        self.err(
                            f"negative integer literal -{mag} does not fit {expected}",
                            e,
                        )
                    x.ty = expected
                    self.expr_types[id(x)] = expected
                    xt = expected
                else:
                    xt = self.expr(
                        x,
                        (
                            ...
                            and (
                                int_info(expected)
                                or expected.kind == "name"
                                and expected.a[0][0] in FLOATS
                            )
                            else None
                        ),
                    )
                if opx == "!":
                    self.req(name_ty("bool"), xt, e)
                    t = xt
                elif opx == "~":
                    if not int_info(xt):
                        self.err("~ requires integer", e)
                    t = xt
                elif opx == "-":
                    if not (
                        int_info(xt) or (xt.kind == "name" and xt.a[0][0] in FLOATS)
                    ):
                        self.err("- requires numeric", e)
                    t = xt
        elif k == "is":
            subject, pat = e.a
            st = self.expr(subject)
            binds, _ = self.pattern(pat, st)
            if binds:
                self.err(
                    "a binding `is` pattern is only allowed as the complete if/while condition",
                    e,
                )
            t = name_ty("bool")
        elif k == "binary":
            ...
            x, to = e.a
            to = self.resolve_ty(to)
            fr = self.expr(x)
            self.check_cast(fr, to, e)
            t = to
        elif k == "new":
            inner_expected = (
                expected.a[0] if expected and expected.kind == "ref" else None
            )
            ...
            at = self.expr(a)
            if at.kind != "array":
                self.err("indexing requires []T", e)
            self.req(name_ty("u64"), self.expr(ix, name_ty("u64")), ix)
            t = at.a[0]
        elif k == "field":
            # parser mostly folds dotted chains into qname; kept for completeness
            bt = self.expr(e.a[0])
            t = self.field_type(bt, e.a[1], e)
        elif k == "structlit":
            t = self.struct_lit(e, expected)
        elif k == "call":
            t = self.call_type(e, expected)
        elif k == "generic_call":
            callee, type_args, call_args = e.a
            # Resolve callee: must be a named function
            if callee.kind != "qname" or len(callee.a[0]) != 1:
                self.err("generic call syntax requires a direct function name", e)
            name = callee.a[0][0]
            if name not in self.funcs:
                self.err(f"unknown function {name}", e)
            f = self.funcs[name]
            resolved_args = tuple(self.resolve_ty(t) for t in type_args)
            if len(resolved_args) != len(f.gps):
                self.err(
                    f"{name} expects {len(f.gps)} type arguments, got {len(resolved_args)}",
                    e,
                )
            m = dict(zip(f.gps, resolved_args))
            # Check the call arguments against substituted parameter types
            if len(call_args) != len(f.params):
                self.err(
                    f"{name} expects {len(f.params)} arguments, got {len(call_args)}",
                    e,
                )
            for (_, p), a in zip(f.params, call_args):
                subst_p = substitute(p, m)
                self.req(subst_p, self.expr(a, subst_p), a)
            e.generic_args = resolved_args
            t = substitute(f.ret, m)
        elif k == "anonfn":
            t = self.anon_type(e, expected)
        else:
            self.err(f"unhandled expression {k}", e)
        e.ty = t
        self.expr_types[id(e)] = t
        if expected is not None:
            self.req(expected, t, e)
        return t
    ...
    def qname_expr(self, q, node, expected=None):
        ...
        if t is not None:
            for f in q[1:]:
                t = self.field_type(t, f, node)
            return t
        if len(q) == 1 and first in self.consts:
            return self.consts[first].ty
        if len(q) == 1 and first in self.funcs:
            f = self.funcs[first]
            if f.gps:
                self.err(f"generic function {first} is not a first-class value", node)
    ...
    def field_type(self, t: Ty, name, node):
        ...
        if (
            base.kind != "name"
            or len(base.a[0]) != 1
            or base.a[0][0] not in self.structs
        ):
            self.err(f"{t} has no field {name}", node)
        s = self.structs[base.a[0][0]]
        fs = {n: (pub, ty) for pub, n, ty, _ in s.fields}
        if name not in fs:
            self.err(f"{s.name} has no field {name}", node)
        pub, fty = fs[name]
        owner = s.owner
        if not pub and self.current_origin != owner:
            self.err(f"field {name} of {s.name} is private", node)
        return substitute(
            self._resolved_decl_ty(fty, s.gps), dict(zip(s.gps, base.a[1]))
        )
    ...
    def struct_lit(self, e, expected):
        ...
        n = cal.a[0][0]
        if n not in self.structs:
            self.err(f"{n} is not a struct", e)
        s = self.structs[n]
        supplied = {}
        for fn, x in fields:
            if fn in supplied:
                self.err(f"duplicate struct literal field {fn}", e)
            supplied[fn] = x
        ...
        if self.current_origin != owner:
            ...
            if private:
                self.err(
                    f"cannot construct {n} outside its module because it has private fields",
                    e,
                )
        if set(supplied) != set(declared):
            self.err(
                f"struct literal fields for {n} must be exactly {sorted(declared)}", e
            )
        m = {}
        if (
            expected
            and expected.kind == "name"
            and expected.a[0] == (n,)
            and len(expected.a[1]) == len(s.gps)
        ):
            m.update(zip(s.gps, expected.a[1]))
        # infer type params from field values where possible
        for fn, tmpl in declared.items():
            tmpl = self._resolved_decl_ty(tmpl, s.gps)
            at = self.expr(
                supplied[fn],
                substitute(tmpl, m) if self.is_fully_bound(tmpl, m) else None,
            )
            self.unify(tmpl, at, m, e)
        if set(m) != set(s.gps):
            self.err(f"cannot infer type arguments for {n}; add an expected type", e)
        actual = name_ty(n, [m[g] for g in s.gps])
        for fn, tmpl in declared.items():
            self.req(
                substitute(self._resolved_decl_ty(tmpl, s.gps), m),
                self.expr(
                    supplied[fn], substitute(self._resolved_decl_ty(tmpl, s.gps), m)
                ),
                supplied[fn],
            )
        return actual
    ...
    def call_type(self, e, expected):
        ...
        if cal.kind == "qname" and len(cal.a[0]) == 1 and cal.a[0][0] in self.builtins:
            ...
            if n == "len":
                if len(args) != 1:
                    self.err("len expects one argument", e)
                at = self.expr(args[0])
                if at.kind != "array":
                    self.err("len expects []T", args[0])
                return name_ty("u64")
            if n == "push":
                if len(args) != 2:
                    self.err("push expects two arguments", e)
                at = self.expr(args[0])
                if not is_mutable_array(at):
                    if is_const_array(at):
                        self.err("push requires mutable []T, got const []T", args[0])
                    self.err("push first argument must be []T", args[0])
                self.req(at.a[0], self.expr(args[1], at.a[0]), args[1])
                return UNIT
            if n == "pop":
                if len(args) != 1:
                    self.err("pop expects one argument", e)
                at = self.expr(args[0])
                if not is_mutable_array(at):
                    if is_const_array(at):
            ...
            if n == "splice":
                if len(args) != 4:
                    self.err("splice expects array, start, end, replacement", e)
                at = self.expr(args[0])
                if not is_mutable_array(at):
                    if is_const_array(at):
                ...
                replacement_ty = const_arr(at.a[0])
                self.req(
                    replacement_ty,
        ...
        if cal.kind == "qname" and len(cal.a[0]) == 1 and cal.a[0][0] in self.funcs:
            f = self.funcs[cal.a[0][0]]
            return self.instantiate_call(f, args, expected, e)
        # enum constructor
        if cal.kind == "qname":
            ...
            if v is not None:
                et, pts, m = v
                if len(pts) != len(args):
                    self.err(f"enum constructor expects {len(pts)} arguments", e)
                for p, a in zip(pts, args):
                    self.req(p, self.expr(a, p), a)
                e.resolved_variant = (et, cal.a[0][-1])
                return et
        # ordinary function value (including struct field / anon fn)
        ct = self.expr(cal)
        if ct.kind != "fn":
            self.err(f"called value has type {ct}, not a function", cal)
        ps, r = ct.a
        if len(ps) != len(args):
            self.err(f"function expects {len(ps)} arguments, got {len(args)}", e)
    ...
    def instantiate_call(self, f: FnInfo, args, expected, node):
        ...
        m = {}
        if expected:
            self.unify(f.ret, expected, m, node, soft=True)
        ...
        if set(m) != set(f.gps):
            self.err(f"cannot infer generic arguments for {f.name}", node)
        node.generic_args = tuple(m[g] for g in f.gps)
        for (_, p), a in zip(f.params, args):
            self.req(substitute(p, m), self.expr(a, substitute(p, m)), a)
        # direct recursive generic call must preserve exact type params
        if self.current_fn is f:
            for gp in f.gps:
                if m[gp] != tparam(gp):
                    self.err(
                        f"recursive generic call to {f.name} changes type parameter {gp}",
                        node,
                    )
    ...
    def is_fully_bound(self, t, m):
        ...
        if t.kind == "param":
            return t.a[0] in m
        if t.kind == "name":
            return all(self.is_fully_bound(x, m) for x in t.a[1])
        if t.kind in ("opt", "ref", "array"):
            return self.is_fully_bound(t.a[0], m)
        if t.kind == "fn":
            return all(
                self.is_fully_bound(x, m) for x in t.a[0]
            ) and self.is_fully_bound(t.a[1], m)
        return True
    ...
    def unify(self, p: Ty, a: Ty, m, node, soft=False):
        ...
        if p.kind in ("opt", "ref", "array"):
            return self.unify(p.a[0], a.a[0], m, node, soft)
        if p.kind == "fn":
            if len(p.a[0]) != len(a.a[0]):
                if soft:
                    return False
                self.err("function arity differs during generic inference", node)
            ok = all(self.unify(x, y, m, node, soft) for x, y in zip(p.a[0], a.a[0]))
            return self.unify(p.a[1], a.a[1], m, node, soft) and ok
        return True
    ...
    def resolve_variant_qname(self, q, expected, args):
        ...
        if v not in variants:
            return None
        pts = tuple(self._resolved_decl_ty(x, info.gps) for x in variants[v])
        m = {}
        if (
            expected
            and expected.kind == "name"
            and expected.a[0] == (en,)
            and len(expected.a[1]) == len(info.gps)
        ):
            m.update(zip(info.gps, expected.a[1]))
        if args:
            if len(args) != len(pts):
                self.err(f"{en}.{v} expects {len(pts)} payloads")
            for p, a in zip(pts, args):
                ep = substitute(p, m) if self.is_fully_bound(p, m) else None
                at = self.expr(a, ep)
                self.unify(p, at, m, a)
        if set(m) != set(info.gps):
            if info.gps:
                return None
        et = name_ty(en, [m[g] for g in info.gps])
    ...
    def anon_type(self, e, expected):
        ...
        old_reserved = self.source_reserved
        old_loop = self.loop
        ...
        for n, t in ps:
            ...
            self.bind(n, t, e)
        self.block(b, False)
        if self.ret != UNIT and not self.block_returns(b):
            self.err("anonymous function may fall off end", e)
        self.pop()
        self.scopes = old_scopes
        self.ret = old_ret
        self.current_fn = old_fn
        self.source_reserved = old_reserved
        self.loop = old_loop
        t = fnty(rps, self.resolve_ty(rt))
        self.anon_types[id(e)] = t
        return t
    ...
    def fieldless_enum(self, t):
        ...
        return info is not None and all(
            len(payload) == 0 for _, payload in info.variants
        )
    ...
    def binop(self, opx, l, r, node):
        ...
        if opx in CMP:
            self.req(l, r, node)
            if opx in ("<", "<=", ">", ">=") and not (
                int_info(l) or (l.kind == "name" and l.a[0][0] in FLOATS)
            ):
                self.err("ordering only supports numeric types", node)
            if opx in ("==", "!="):
                ok = (
                    is_prim(l, "bool")
                    or int_info(l)
                    or (l.kind == "name" and l.a[0][0] in FLOATS)
                    or l.kind == "ref"
                    or self.fieldless_enum(l)
                )
                if not ok:
                    self.err(f"equality is not defined for {l}", node)
            return name_ty("bool")
        if opx in ("<<", ">>"):
            if not int_info(l):
                self.err("shift left operand must be integer", node)
            self.req(name_ty("u64"), r, node)
            return l
        if opx in ("&", "|", "^"):
            if not int_info(l):
                self.err("bitwise operator requires integers", node)
            self.req(l, r, node)
            return l
        if opx in ("+", "-", "*", "/", "%"):
            if not (int_info(l) or (l.kind == "name" and l.a[0][0] in FLOATS)):
                self.err("arithmetic requires numeric type", node)
            if opx == "%" and l.kind == "name" and l.a[0][0] in FLOATS:
                self.err("% is integer-only", node)
            self.req(l, r, node)
            return l
    ...
    def place(self, e):
        ...
        if e.kind == "qname":
            ...
            if t is None:
                self.err("assignment target must start with a local variable", e)
            for f in q[1:]:
                t = self.field_type(t, f, e)
            e.ty = t
            return t
        if e.kind == "index":
            at = self.expr(e.a[0])
            if is_const_array(at):
                self.err("cannot assign through const []T", e)
            if at.kind != "array":
                self.err("indexed assignment requires []T", e)
            self.req(name_ty("u64"), self.expr(e.a[1], name_ty("u64")), e.a[1])
            return at.a[0]
        if e.kind == "unary" and e.a[0] == "*":
            rt = self.expr(e.a[1])
            if rt.kind != "ref":
                self.err("* assignment requires ref T", e)
            return rt.a[0]
        if e.kind == "field":
            try:
                bt = self.place(e.a[0])
                return self.field_type(bt, e.a[1], e)
            except LangError:
                bt = self.expr(e.a[0])
                if bt.kind == "ref":
                    return self.field_type(bt, e.a[1], e)
                self.err(
                    "field assignment requires an assignable struct or a ref-to-struct value",
                    e,
                )
    ...
    def eval_const(self, n):
        ...
        if c.state == 2:
            return c.value
        if c.state == 1:
            self.err(f"constant cycle involving {n}", c.expr)
        c.state = 1
        old = self.scopes
        self.scopes = []
        val, ty = self.const_expr(c.expr, c.ty)
        self.req(c.ty, ty, c.expr)
        c.value = val
        c.state = 2
        self.scopes = old
        return val
    ...
    def const_expr(self, e, expected=None):
        ...
        if k == "byte":
            return e.a[0], name_ty("u8")
        if k == "int":
            t = (
                expected
                if expected is not None and int_info(expected)
                else name_ty("i64")
            )
            v = parse_int_text(e.a[0])
            if int_info(t):
                w, sg = int_info(t)
                lo = -(1 << (w - 1)) if sg else 0
                hi = (1 << (w - 1)) - 1 if sg else (1 << w) - 1
                if not (lo <= v <= hi):
                    self.err(f"integer literal {v} does not fit {t}", e)
            return v, t
        if k == "float":
        ...
        if k == "qname" and len(e.a[0]) == 1 and e.a[0][0] in self.consts:
            n = e.a[0][0]
            return self.eval_const(n), self.consts[n].ty
        if k == "unary" and e.a[0] in ("-", "~", "!"):
            v, t = self.const_expr(e.a[1], expected)
            opx = e.a[0]
            if opx == "-":
                return -v, t
            if opx == "~":
            ...
            return (not v), t
        if k == "binary":
            lv, lt = self.const_expr(e.a[1], expected)
            rhs_exp = name_ty("u64") if e.a[0] in ("<<", ">>") else lt
            rv, rt = self.const_expr(e.a[2], rhs_exp)
            out = self.binop(e.a[0], lt, rt, e)
            return self.eval_scalar_op(e.a[0], lv, rv, lt), out
        if k == "cast":
            v, t = self.const_expr(e.a[0])
            to = self.resolve_ty(e.a[1])
        ...
        if int_info(to):
            if isinstance(v, float):
                if not math.isfinite(v):
                    raise fault("float-to-int cast out of range")
                v = math.trunc(v)
                w, s = int_info(to)
                lo = -(1 << (w - 1)) if s else 0
                hi = (1 << (w - 1)) - 1 if s else (1 << w) - 1
    ...
    def external_member_type(self, alias, rest, node):
        ...
        if mod is None:
            ...
            if hm is None:
                self.err(f'unresolved module {".".join(q)}', node)
    ...
    def member_type(self, rest, node=None, external=False):
        ...
        if len(rest) != 1:
            raise LangError(
                ...
                node.span if isinstance(node, N) else None,
            )
        n = rest[0]
        c = self.checker
        if n in c.consts:
            if external and not c.consts[n].public:
                raise LangError(f"{n} is private")
            return c.consts[n].ty
        if n in c.funcs:
            f = c.funcs[n]
            if external and not f.public:
                raise LangError(f"{n} is private")
            if f.gps:
                raise LangError("generic imported function values are not first-class")
            return fnty([t for _, t in f.params], f.ret)
...
class Interpreter:
    ...
    def __init__(self, checked: CheckedModule, host_modules: dict | None = None):
        ...
        self.frames = []
        self.anon = {}
        self.live_refs = weakref.WeakSet()
    ...
    def stmt(self, s):
        ...
        if k == "forin":
            ...
            while i < len(a.items):
                broke = False
                self.pushframe({n: copy_value(a.items[i])})
                try:
                    self.block(b, False)
                except ContinueSig:
                    pass
    ...
    def match(self, p, v, t):
        ...
        if k == "p_wild":
            return {}
        if k == "p_bind":
            return {p.a[0]: copy_value(v)}
        if k == "p_unit":
            return {} if v is UNITV else None
        if k == "p_none":
            return {} if v is None else None
        if k == "p_some":
            return (
                None
                if not isinstance(v, SomeVal)
                else self.match(p.a[0], v.value, t.a[0])
            )
        if k == "p_bool":
            return {} if v is p.a[0] else None
        if k == "p_int":
            return {} if v == parse_int_text(p.a[0]) else None
        if k == "p_byte":
            return {} if v == p.a[0] else None
        if k == "p_name":
            q, subs = p.a
            variant = self.c._variant_for_pattern(q, t)
            if variant is None and len(q) == 1 and not subs:
                return {q[0]: copy_value(v)}
            if variant is None:
                return None
            en, vn, pts = variant
            if not isinstance(v, EnumVal) or v.name != en or v.variant != vn:
                return None
            out = {}
    ...
    def eval(self, e):
        ...
        if k in ("bool", "byte"):
            return e.a[0]
        if k == "int":
            return wrap_int(parse_int_text(e.a[0]), e.ty)
        if k == "float":
            return fround(float(e.a[0].replace("_", "")), e.ty)
        if k == "string":
            return self.alloc_array(e.a[0])
        if k == "none":
            return None
        if k == "some":
            return SomeVal(copy_value(self.eval(e.a[0])))
        if k == "qname":
            return self.qvalue(e.a[0], e)
        if k == "array":
            return self.alloc_array(self.eval(x) for x in e.a[0])
        if k == "repeat":
            v = self.eval(e.a[0])
            n = self.eval(e.a[1])
            return self.alloc_array(copy_value(v) for _ in range(n))
        if k == "unary":
            opx, x = e.a
            if opx == "*":
                return copy_value(self.eval(x).value)
            v = self.eval(x)
            if opx == "!":
                return not v
            if opx == "~":
                return wrap_int(~v, e.ty)
            if opx == "-":
                return wrap_int(-v, e.ty) if int_info(e.ty) else fround(-v, e.ty)
        if k == "is":
            v = self.eval(e.a[0])
            return self.match(e.a[1], v, e.a[0].ty) is not None
        if k == "binary":
            opx, l, r = e.a
            if opx == "&&":
                a = self.eval(l)
                return bool(a and self.eval(r))
            if opx == "||":
                a = self.eval(l)
                return bool(a or self.eval(r))
            return self.scalar_op(opx, self.eval(l), self.eval(r), l.ty)
        if k == "cast":
            return self.c.cast_value(self.eval(e.a[0]), e.a[0].ty, e.ty, TrapSig)
        if k == "new":
            return self.alloc_ref(self.eval(e.a[0]))
        if k == "index":
            a = self.eval(e.a[0])
            i = self.eval(e.a[1])
        ...
        if k == "field":
            return copy_value(self.place(e).get())
        if k == "structlit":
            cal, fs = e.a
            n = cal.a[0][0]
            return StructVal(n, {fn: copy_value(self.eval(x)) for fn, x in fs})
        if k == "generic_call":
            callee, type_args, call_args = e.a
            if callee.kind == "qname" and len(callee.a[0]) == 1:
                n = callee.a[0][0]
                if n in self.c.funcs:
                    return self.call_user(
                        self.c.funcs[n],
                        [self.eval(a) for a in call_args],
                    )
            raise TrapSig(f"cannot evaluate generic_call for {callee}")
        if k == "call":
            cal, args = e.a
            if cal.kind == "qname" and len(cal.a[0]) == 1:
                n = cal.a[0][0]
                if n == "len":
                    return len(self.eval(args[0]).items)
                if n == "push":
                    self.eval(args[0]).items.append(copy_value(self.eval(args[1])))
                    return UNITV
                if n == "pop":
                    a = self.eval(args[0])
                    if not a.items:
                        raise TrapSig("pop from empty array")
                    return copy_value(a.items.pop())
                if n == "splice":
                    a = self.eval(args[0])
                    start = self.eval(args[1])
                    end = self.eval(args[2])
                    repl = self.eval(args[3])
                    if start < 0 or end < start or end > len(a.items):
                        raise TrapSig("splice range out of bounds")
                    snap = [copy_value(x) for x in repl.items]
                    a.items[start:end] = snap
                    return UNITV
                if n in self.c.funcs:
                    return self.call_user(self.c.funcs[n], [self.eval(a) for a in args])
            if hasattr(e, "resolved_variant"):
                et, vn = e.resolved_variant
                return EnumVal(et.a[0][0], vn, [copy_value(self.eval(a)) for a in args])
            fv = self.eval(cal)
            av = [self.eval(a) for a in args]
            return self.call_value(fv, av)
        if k == "anonfn":
            self.anon[id(e)] = e
            return AnonFnVal(id(e))
    ...
    def qvalue(self, q, e):
        ...
        try:
            ...
            start = 1
            for f in q[start:]:
                v = self.get_field(v, f)
            return copy_value(v)
        except TrapSig:
            pass
        if len(q) == 1 and q[0] in self.top_consts:
            return self.top_consts[q[0]]
        if len(q) == 1 and q[0] in self.c.funcs:
            return UserFnVal(q[0])
        if hasattr(e, "resolved_variant"):
            et, vn = e.resolved_variant
            return EnumVal(et.a[0][0], vn, [])
        if q[0] in self.c.imports:
            modq = self.c.imports[q[0]]
            hm = self.host_modules.get(modq)
            if hm:
                return hm.get_value(q[1:])
    ...
    def call_value(self, f, args):
        ...
        if isinstance(f, AnonFnVal):
            e = self.anon[f.node_id]
            ps, rt, b = e.a
            self.pushframe({n: copy_value(v) for (n, _), v in zip(ps, args)})
            try:
                self.block(b, False)
                return UNITV
            except ReturnSig as r:
    ...
    def get_field(self, v, f):
        ...
        if not isinstance(v, StructVal):
            raise TrapSig("field on non-struct")
        return v.fields[f]
        ...
        if e.kind == "qname":
            ...
            for fr in reversed(self.frames):
                if q[0] in fr:
                    ...
                    break
            if owner is None:
                raise TrapSig("place does not start with local")
            if len(q) == 1:
                return Place(lambda: owner[q[0]], lambda v: owner.__setitem__(q[0], v))
            cur = owner[q[0]]
            for f in q[1:-1]:
                if isinstance(cur, RefObj):
                    cur = cur.value
                cur = cur.fields[f]
            last = q[-1]
            if isinstance(cur, RefObj):
                cur = cur.value
            return Place(
                lambda: cur.fields[last], lambda v: cur.fields.__setitem__(last, v)
            )
        if e.kind == "index":
            a = self.eval(e.a[0])
            i = self.eval(e.a[1])
...
def copy_value(v):
    ...
    if (
        ...
        or v is None
        or isinstance(
            v, (bool, int, float, ArrayObj, RefObj, UserFnVal, AnonFnVal, HostFnVal)
        )
    ):
        return v
    if isinstance(v, SomeVal):
        return SomeVal(copy_value(v.value))
    if isinstance(v, StructVal):
        return StructVal(v.name, {k: copy_value(x) for k, x in v.fields.items()})
    if isinstance(v, EnumVal):
        return EnumVal(v.name, v.variant, [copy_value(x) for x in v.payload])
    return v
...
class Program:
    ...
    def __init__(
        ...
        self.checked = Checker(
            self.merged, ("__linked__",), host_modules=self.host_modules
        ).checked()
        ...
        def dfs(m, path):
            ...
            for q in self.imports[m].values():
                if q in self.sources:
                    dfs(q, path + [m])
                elif q not in self.host_modules:
                    raise LangError(f'unresolved module {".".join(q)}')
            state[m] = 2
            out.append(m)
        ...
        for m in self.order:
            for alias, target in self.imports[m].items():
                if target in self.host_modules:
                    hd = N("import", (target, internal_name(m, alias)), None)
                    hd.origin = m
                    ds.append(hd)
            for d in self.parsed[m].a[0]:
                ...
                rd = self._decl(m, d)
                rd.origin = m
                rd.reserved = set(self.tops[m]) | set(self.imports[m])
                ds.append(rd)
    ...
    def _resolve_source_name(self, m, q, node=None):
        ...
        if first in self.imports[m]:
            ...
            if target in self.sources:
                if len(q) < 2:
                    raise LangError(
                        "source module name is not a runtime value",
                        node.span if isinstance(node, N) else None,
                    )
                member = q[1]
                if member not in self.tops[target]:
                    raise LangError(f'unknown member {member} of {".".join(target)}')
                if member not in self.exports[target]:
                    raise LangError(f'{".".join(target)}.{member} is private')
                return (internal_name(target, member),) + q[2:]
            return (internal_name(m, first),) + q[1:]  # unique internal host alias
        if first in self.tops[m]:
            return (internal_name(m, first),) + q[1:]
        return q
    ...
    def _ty(self, m, t, gps=frozenset()):
        ...
        if t.kind == "name":
            q, args = t.a
            if len(q) == 1 and q[0] in gps:
                return t
            return Ty(
                "name",
                (
                    self._resolve_source_name(m, q),
                    tuple(self._ty(m, x, gps) for x in args),
                ),
            )
        if t.kind in ("opt", "ref", "array"):
            return Ty(t.kind, (self._ty(m, t.a[0], gps),))
        if t.kind == "fn":
            return fnty([self._ty(m, x, gps) for x in t.a[0]], self._ty(m, t.a[1], gps))
        return t
    ...
    def _expr(self, m, e, gps=frozenset()):
        ...
        k = e.kind
        if k == "qname":
            return N(k, (self._resolve_source_name(m, e.a[0], e),), e.span)
        if k == "cast":
            return N(k, (self._expr(m, e.a[0], gps), self._ty(m, e.a[1], gps)), e.span)
        if k == "is":
            return N(
                k, (self._expr(m, e.a[0], gps), self._pattern(m, e.a[1], gps)), e.span
            )
        if k == "anonfn":
            ps, rt, b = e.a
            return N(
                k,
                (
                    tuple((n, self._ty(m, t, gps)) for n, t in ps),
                    self._ty(m, rt, gps),
                    tuple(self._stmt(m, s, gps) for s in b),
                ),
                e.span,
            )
        ...
        for x in e.a:
            if isinstance(x, N):
                aa.append(self._expr(m, x, gps))
            elif isinstance(x, Ty):
                aa.append(self._ty(m, x, gps))
            elif isinstance(x, tuple):
                ys = []
                for y in x:
                    if isinstance(y, N):
                        ys.append(self._expr(m, y, gps))
                    elif isinstance(y, tuple) and len(y) == 2 and isinstance(y[1], N):
                        ys.append((y[0], self._expr(m, y[1], gps)))
                    else:
                        ys.append(y)
                aa.append(tuple(ys))
            else:
    ...
    def _pattern(self, m, p, gps=frozenset()):
        ...
        if p.kind == "p_name":
            ...
            return N(
                "p_name", (rq, tuple(self._pattern(m, x, gps) for x in subs)), p.span
            )
        return N(
            p.kind,
            tuple(self._pattern(m, x, gps) if isinstance(x, N) else x for x in p.a),
            p.span,
        )
    ...
    def _stmt(self, m, s, gps=frozenset()):
        ...
        if k == "var":
            return N(
                ...
                (
                    s.a[0],
                    self._ty(m, s.a[1], gps) if s.a[1] else None,
                    self._expr(m, s.a[2], gps),
                ),
                s.span,
            )
        if k == "assign":
            return N(
                k,
                (self._expr(m, s.a[0], gps), s.a[1], self._expr(m, s.a[2], gps)),
                s.span,
            )
        if k == "exprstmt":
            return N(k, (self._expr(m, s.a[0], gps),), s.span)
        if k in ("if", "while"):
            c = s.a[0]
            if c.kind == "is":
                c = N(
                    "is",
                    (self._expr(m, c.a[0], gps), self._pattern(m, c.a[1], gps)),
                    c.span,
                )
            else:
                c = self._expr(m, c, gps)
            if k == "if":
                return N(
                    k,
                    (
                        c,
                        tuple(self._stmt(m, x, gps) for x in s.a[1]),
        ...
        if k == "for":
            ...
            if c:
                c = (
                    ...
                    if c.kind == "is"
                    else self._expr(m, c, gps)
                )
            return N(
                k,
                (
                    self._stmt(m, init, gps) if init else None,
                    c,
                    self._stmt(m, step, gps) if step else None,
                    tuple(self._stmt(m, x, gps) for x in b),
                ),
                s.span,
            )
        if k == "forin":
        ...
        if k == "match":
            return N(
                k,
                (
                    self._expr(m, s.a[0], gps),
                    tuple(
                        (
                            self._pattern(m, p, gps),
                            tuple(self._stmt(m, x, gps) for x in b),
                        )
                        for p, b in s.a[1]
                    ),
        ...
        if k == "return":
            return N(k, (self._expr(m, s.a[0], gps) if s.a[0] else None,), s.span)
    ...
    def _decl(self, m, d):
        ...
        if k == "const":
            pub, n, t, e = d.a
            return N(
                k, (pub, internal_name(m, n), self._ty(m, t), self._expr(m, e)), d.span
            )
        if k == "struct":
            pub, n, gps, fs = d.a
            reserved = set(self.tops[m]) | set(self.imports[m])
            if set(gps) & reserved:
                raise LangError("generic parameter shadows module name")
            gs = frozenset(gps)
            out = N(
                k,
                (
                    ...
                    internal_name(m, n),
                    gps,
                    tuple((fp, fn, self._ty(m, t, gs), sp) for fp, fn, t, sp in fs),
                ),
                d.span,
            )
            return out
        if k == "enum":
            ...
            return N(
                ...
                (
                    ...
                    internal_name(m, n),
                    gps,
                    tuple((v, tuple(self._ty(m, t, gs) for t in ts)) for v, ts in vs),
                ),
                d.span,
            )
        if k == "fn":
            pub, n, gps, ps, rt, b = d.a
            reserved = set(self.tops[m]) | set(self.imports[m])
            if set(gps) & reserved:
                raise LangError("generic parameter shadows module name")
            gs = frozenset(gps)
            return N(
                ...
                (
                    ...
                    internal_name(m, n),
                    gps,
                    tuple((pn, self._ty(m, t, gs)) for pn, t in ps),
                    self._ty(m, rt, gs),
                    tuple(self._stmt(m, s, gs) for s in b),
                ),
                d.span,
            )
...
class HostModule:
    ...
    def resolve_type(self, rest, args):
        ...
        if args:
            raise LangError("host opaque types do not take type arguments")
    ...
    def member_type(self, rest, node=None):
        ...
        n = rest[0]
        if n in self.func_types:
            return self.func_types[n]
        if n in self.const_types:
            return self.const_types[n]
        raise LangError(
            f"unknown host member {n}", node.span if isinstance(node, N) else None
        )
    ...
    def get_value(self, rest):
        ...
        n = rest[0]
        if n in self.func_values:
            return self.func_values[n]
        if n in self.const_values:
            return self.const_values[n]
