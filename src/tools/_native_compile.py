...
HOST = {
    ...
    (("linux", "fd"), "read"): 31,
    (("linux", "fd"), "write"): 32,
    (("linux", "process"), "spawn_exact"): 33,
    (("linux", "process"), "spawn_child"): 34,
    (("linux", "process"), "spawn_error"): 35,
    (("linux", "process"), "group"): 36,
    (("linux", "process"), "wait_exit"): 37,
    (("linux", "process"), "exit_code"): 38,
    (("linux", "process"), "term_signal"): 39,
    (("linux", "process"), "send"): 40,
    (("linux", "process"), "send_group"): 41,
    (("linux", "process"), "sigint"): 42,
    (("linux", "process"), "sigquit"): 43,
    (("linux", "process"), "sigterm"): 44,
    (("linux", "process"), "sigkill"): 45,
    (("linux", "process"), "sigstop"): 46,
    (("linux", "process"), "sigtstp"): 47,
    (("linux", "process"), "sigcont"): 48,
    (("linux", "process"), "sighup"): 49,
    (("linux", "fs"), "cwd"): 50,
    (("linux", "fs"), "chdir"): 51,
    (("linux", "env"), "get"): 52,
    (("linux", "env"), "entries"): 53,
    (("linux", "env"), "set"): 54,
    (("linux", "env"), "unset"): 55,
}
...
class NativeEmitter:
    def __init__(self, program: Program, entry: str):
        ...
        self.patterns = []
        self.pattern_aux = []
        self.func_names = list(self.bc.funcs)
        self.fid = {n: i for i, n in enumerate(self.func_names)}
        self.func_slots = {}
        self._prepare_slots()
    ...
    def names_in_pattern(self, p: N, out: set[str]):
        if p.kind == "p_bind":
            ...
            return
        if p.kind == "p_some":
            self.names_in_pattern(p.a[0], out)
            return
        if p.kind == "p_name":
            q, subs = p.a
    ...
    def _prepare_slots(self):
        for name, f in self.bc.funcs.items():
            ...
            for p in f.params:
                add(p)
            for ins in f.code:
                if ins[0] in ("DECL", "LOAD", "LOCAL_PLACE", "LOCAL_INC_U64"):
                    add(ins[1])
                if ins[0] == "TRY_PATTERN":
                    ns = set()
                    self.names_in_pattern(ins[1], ns)
                    for n in sorted(ns):
                        add(n)
    ...
    def pattern(self, p: N, t: Ty, slots: dict[str, int]) -> int:
        ...
        if k == "p_wild":
            ...
            rec = (PK[k], slots[p.a[0]], 0, 0, None, 0)
        elif k == "p_unit" or k == "p_none":
            rec = (PK[k], 0, 0, 0, None, 0)
        elif k == "p_bool":
            rec = (PK[k], 1 if p.a[0] else 0, 0, 0, None, 0)
        elif k == "p_int":
            rec = (PK[k], 0, 0, 0, None, parse_int_text(p.a[0]) & ((1 << 64) - 1))
        elif k == "p_byte":
            rec = (PK[k], 0, 0, 0, None, p.a[0])
        elif k == "p_some":
            child = self.pattern(p.a[0], t.a[0], slots)
            rec = (PK[k], child, 0, 0, None, 0)
        elif k == "p_name":
            q, subs = p.a
            v = self.c._variant_for_pattern(q, t)
            if v is None:
                if len(q) == 1 and not subs:
                    rec = (PK["p_bind"], slots[q[0]], 0, 0, None, 0)
                else:
                ...
                auxid = None
                if child:
                    auxid = len(self.pattern_aux)
                    self.pattern_aux.append(child)
                rec = (PK["p_enum"], self.sid(en), self.sid(vn), len(child), auxid, 0)
        else:
            raise LangError("native pattern " + k)
    ...
    def transform(self, fname: str, ins: tuple):
        ...
        if op == "PUSH_UNIT":
            ...
            if v is None:
                ...
                d.update(op=OP["PUSH_BOOL"], a=int(v))
            elif isinstance(v, int):
                d.update(op=OP["PUSH_INT"], a=TY["u64"], u=v & ((1 << 64) - 1))
            elif isinstance(v, float):
                d.update(op=OP["PUSH_FLOAT"], a=TY["f64"], f=v)
            else:
                raise LangError(f"native constant unsupported {v!r}")
        elif op == "PUSH_INT":
            d.update(u=ins[1] & ((1 << 64) - 1), a=self.ty(ins[2]))
        elif op == "PUSH_FLOAT":
            d.update(f=float(ins[1]), a=self.ty(ins[2]))
        elif op == "MAKE_BYTES":
            d.update(blob=self.blob(ins[1]))
        elif op in (
            "MAKE_SOME",
            "DUP",
            "POP",
            ...
            pass
        elif op in ("DECL", "LOAD", "LOCAL_PLACE", "LOCAL_INC_U64"):
            d["a"] = slots[ins[1]]
        elif op in ("FIELD_PLACE", "VALUE_FIELD_PLACE", "GET_FIELD"):
            d["a"] = self.sid(ins[1])
        elif op in ("INDEX_PLACE", "DEREF_PLACE", "LOAD_PLACE", "STORE_PLACE", "LEN"):
            pass
        elif op == "UNARY":
            d.update(a=UOP[ins[1]], b=self.ty(ins[2]))
        elif op == "BIN":
            d.update(a=BOP[ins[1]], b=self.ty(ins[2]))
        elif op == "CAST":
            d.update(a=self.ty(ins[1]), b=self.ty(ins[2]))
        elif op == "MAKE_STRUCT":
            d.update(
                a=self.sid(ins[1]),
                b=len(ins[2]),
                aux=self.auxints(self.sid(x) for x in ins[2]),
            )
        elif op == "MAKE_ENUM_ZERO":
            d.update(a=self.sid(ins[1]), b=self.sid(ins[2]))
        elif op == "MAKE_ENUM":
            d.update(a=self.sid(ins[1]), b=self.sid(ins[2]), c=ins[3])
        elif op == "PUSH_FUNC":
            d["a"] = self.fid[ins[1]]
        elif op == "CALL_NAMED":
            d.update(a=self.fid[ins[1]], b=ins[2])
        elif op == "HOST_MEMBER":
            mod = tuple(ins[1])
            rest = tuple(ins[2])
            if len(rest) != 1 or (mod, rest[0]) not in HOST:
                raise LangError(f"native unsupported host member {mod}.{rest}")
            d["a"] = HOST[(mod, rest[0])]
        elif op == "TRY_PATTERN":
            d["a"] = self.pattern(ins[1], ins[2], slots)
        else:
            raise LangError("native missing opcode " + op)
        return d
    ...
    def emit(self) -> str:
        ...
        for i, xs in enumerate(self.aux):
            A(f'static const int aux_{i}[] = {{{",".join(map(str,xs))}}};')
        for i, xs in enumerate(self.pattern_aux):
            A(f'static const int paux_{i}[] = {{{",".join(map(str,xs))}}};')
        ...
        for fi, n in enumerate(self.func_names):
            ...
            A(
                f'static const int fn_params_{fi}[] = {{{",".join(map(str,ps))}}};'
                if ps
                else ""
            )
            ...
            for d in funins[n]:
                ...
                for x in ("a", "b", "c"):
                    if x in d:
                        fields.append(f".{x}={d[x]}")
                if "u" in d:
                    fields.append(f'.u=UINT64_C({d["u"]})')
                if "f" in d:
                    f = d["f"]
                    if math.isnan(f):
                        fs = "NAN"
                    elif math.isinf(f):
                        fs = "INFINITY" if f > 0 else "-INFINITY"
                    else:
                        fs = repr(float(f))
                    fields.append(f".f={fs}")
                if "blob" in d:
                    fields.append(f'.ptr=&blob_{d["blob"]}')
                if "aux" in d:
                    fields.append(f'.ptr=aux_{d["aux"]}')
                A("  {" + ",".join(fields) + "},")
            A("};")
        ...
        try:
            ...
            if m:
                ...
                if candidate.is_file():
                    e.path = candidate
                elif tuple(m) == tuple(mod):
                    e.path = entry
            raise
        en = internal_name(mod, "main")
        if en not in p.checked.checker.funcs:
            raise LangError("entry module has no main")
        return p, en
    finally:
        cleanup(ph, th)
...
def build_tool(kind: str):
    if kind == "editor":
        ...
        return p, internal_name(("main",), "main")
    name = {"lsp-l": "slang-lsp", "lsp-json": "json-lsp", "lsp-ini": "ini-lsp"}[kind]
    p = Program(build_sources(SERVER_FILES[name]), {("stdio",): stdio_host()})
    return p, internal_name(("main",), "main")
...
def compile_native(
    ...
    if emit_c:
        ...
        td = tempfile.TemporaryDirectory(prefix="lc-")
        tmp = td
        cpath = Path(td.name) / "program.c"
...
def main():
    ...
    if ns.tool:
        p, e = build_tool(ns.tool)
    else:
        if not ns.source:
            ap.error("source required unless --tool")
