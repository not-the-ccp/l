...
class NativeEmitter:
    ...
    def names_in_pattern(self,p:N,out:set[str]):
        ...
        if p.kind=='p_name':
            q,subs=p.a
            # one-segment no-payload may be binding; conservatively include it;
            # resolved enum names can share a slot name harmlessly because never loaded.
    ...
    def transform(self,fname:str,ins:tuple):
        ...
        return d
    def emit(self)->str:
        # Transform first so tables are complete.
        funins={n:[self.transform(n,i) for i in self.bc.funcs[n].code] for n in self.func_names}
        # Ensure names referenced as function metadata are interned.
...
def default_hosts(args=()):
    ...
    return {('stdio',):stdio_host(),('fs',):fs_host(),('sys',):sys_host(list(args)),('proc',):ph.module(),('term',):th.module()},ph,th

def build_user(entry:Path,root:Path|None=None):
    from sdk_cli import project_sources
