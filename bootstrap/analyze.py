...
def report(project, funcs):
    ...
    by = {}
    for f in funcs:
        by.setdefault(f.module, []).append(f)
    for mod in sorted(by):
        out.append("module " + ".".join(mod))
        for f in by[mod]:
            p = ", ".join(f"{x['name']}: {x['type']}" for x in f.params)
            m = f.metrics
            out.append(
            ...
            if f.calls:
                out.append(
                    "    calls: "
                    + ", ".join(
                        (c.resolved or c.callee) + (" [internal]" if c.internal else "")
                        for c in f.calls
                    )
                )
            if f.unreachable_lines:
                out.append(
                    "    unreachable statements at lines: "
                    + ", ".join(map(str, f.unreachable_lines))
                )
        out.append("")
    return "\n".join(out).rstrip() + "\n"
...
def svg(dot):
    ...
    if exe is None:
        raise LangError(
            "Graphviz 'dot' is required for --format svg; use dot or mermaid output without it"
        )
    p = subprocess.run(
        [exe, "-Tsvg"],
        input=dot,
    ...
    if p.returncode:
        raise LangError("Graphviz failed: " + p.stderr.strip())
    return p.stdout
            ...
            try:
                ...
                if mod:
                    ...
                    if candidate.is_file():
                        e.path = candidate
                    elif tuple(mod) == tuple(entry):
                        e.path = source.resolve()
                raise
        finally:
            cleanup(ph, th)
    if not include_stdlib:
        std = set(stdlib_sources())
        sources = {m: s for m, s in sources.items() if m not in std or m == entry}
...
def modes(ns, p):
    ...
    allowed = {
        ...
        "model": {"json"},
        "ast": {"json"},
        "cfg": {"json", "mermaid", "dot", "svg"},
        "calls": {"json", "mermaid", "dot", "svg"},
    }
    if fmt not in allowed[view]:
        p.error(f"--view {view} does not support --format {fmt}")
    return view, fmt
...
def parser():
    ...
    p.add_argument("--root")
    p.add_argument("--view", choices=["report", "model", "ast", "cfg", "calls"])
    p.add_argument("--format", choices=["text", "json", "mermaid", "dot", "svg"])
    ...
    p.add_argument("--include-stdlib", action="store_true")
...
def main(argv=None):
    ...
    try:
        sources, entry = load_project(source, root, not ns.no_check, ns.include_stdlib)
        project = analyze_project(sources, entry)
        funcs = select_functions(project, ns.module, ns.function)
        if view == "report":
            out = (
                report(project, funcs)
                if fmt == "text"
                else json.dumps(
                    {
                        "entry_module": ".".join(entry),
            ...
            out = json.dumps(project.to_dict(), indent=2, sort_keys=True) + "\n"
        elif view == "ast":
            mods = (
                {f.module for f in funcs}
                if (ns.module or ns.function)
                else set(project.asts)
            )
            out = (
                json.dumps(
                    {
                        "modules": {
                            ".".join(m): ast_value(project.asts[m])
            ...
            if fmt == "json":
                ...
                out = render_cfg_mermaid(funcs)
            else:
                dot = render_cfg_dot(funcs)
                out = svg(dot) if fmt == "svg" else dot
        else:
            if fmt == "json":
                out = (
                    json.dumps(
                        {
                            "edges": [
                                {"caller": a, "callee": b, "internal": i, "count": n}
                                for a, b, i, n in call_edges(funcs)
                            ]
                        },
                        indent=2,
                        sort_keys=True,
                ...
                out = render_calls_mermaid(funcs)
            else:
                dot = render_calls_dot(funcs)
                out = svg(dot) if fmt == "svg" else dot
        if ns.output:
