...
class FunctionAnalysis:
    ...
    def to_dict(self):
        return {
            ...
            "metrics":dict(self.metrics),"calls":[asdict(x) for x in self.calls],"unreachable_lines":list(self.unreachable_lines),
...
def analyze_fn(module,name,params,ret,body,node,source,anonymous=False):
    cfg=CFGBuilder(source); nodes,edges,unreachable,nesting=cfg.build(body); sp=getattr(node,"span",None)
    fn=FunctionAnalysis(module,name,[{"name":n,"type":str(t)} for n,t in params],str(ret),getattr(sp,"line",None),getattr(sp,"end_line",None),
                        metrics(body,nesting),collect_calls(body),nodes,edges,unreachable,anonymous)
    nested=[]
